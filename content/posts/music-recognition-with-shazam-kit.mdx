...
<Callout type="info" emoji="️ℹ️">
  In case of missing service you'll get an error: `The operation couldn't be completed. (com.apple.ShazamKit error 202.)`
</Callout>
...
Calling this function is automatically requested permission to use the microphone.
...
```swift
@Published var library: SHLibrary = .default // [!code highlight]
...
func match() async {
    ...
    case .match(let match):
        if let mediaItem = match.mediaItems.first {
            try? await library.addItems([mediaItem]) // [!code highlight]
        }
    case .noMatch(_):
        print("Mo match")
    case .error(let error, _):
...
`SHLibrary` confirms to `Observable` protocol, so we can observe changes in the library and show them in the interface:
...
struct ShazamView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack {
                List {
                    ForEach(viewModel.library.items) { mediaItem in // [!code highlight]
                        // Media item view
                    }
                }
                shazamButton
...
private var shazamButton: some View {
    Button(action: match) {
        Image(systemName: "shazam.logo.fill")
            .resizable()
            .frame(width: 52, height: 52)
            .symbolEffect(.pulse, isActive: viewModel.managedSession.state == .matching) // [!code highlight]
    }
    .allowsHitTesting(viewModel.managedSession.state != .matching) // [!code highlight]
}
```
...
ShazamKit provides a unique opportunity to recognize music in your app. It's may be useful for apps with audio content:
