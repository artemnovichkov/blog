...
2. A `dataType` property with default value to avoid optoinal values.
...
Here we have a 6 calls of `completionHandler` and weak using of self to avoid retain cycles. Can we improve this code with async/awaits? Let's try it out!
...
Before working with web view content, we must be sure that the main frame navigation completes. We can handle it in `webView(_:didFinish:)` function of `WKNavigationDelegate`. To make this logic async\await-compatible, we will use `withCheckedThrowingContinuation` function.
...
Because `load` function must be called in the main queue, we mark `createData` function with `MainActor` attribute as well. Even more, we can add this attribute to `WebDataManager` class instead:
...
- Better syntax
- Simple work with errors
- Less work with closures and self capturing

I'm still playing with the new concurrency model and collecting useful resources in [awesome-swift-async-await](https://github.com/artemnovichkov/awesome-swift-async-await) repo. Feel free to send a PR with your favorite learning materials related to this topic!
