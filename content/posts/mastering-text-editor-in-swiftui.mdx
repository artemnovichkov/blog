...
description: >-
  Discover the capabilities and limitations of SwiftUI's TextEditor and how to
...
```swift
TextEditor(text: $text, selection: $textSelection)
  .introspect(.textEditor, on: .iOS(.v18, .v26, .v27)) { textView in
      textView.isEditable = false
  }
```

Introspect calls the closure only on listed OS versions, so add new versions as they come out. As of iOS 27, SwiftUI still has no built-in read-only mode for `TextEditor`.
