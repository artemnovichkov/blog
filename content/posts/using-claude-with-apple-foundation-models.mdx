...
- Xcode 27;
- iOS 27 (macOS, visionOS, and watchOS 27 are supported as well);
...
- `.apiKey("...")` — development only. Never ship this in production.
- `.proxied(headers:)` + `baseURL` — production. Your backend adds the credential; the app ships no key. Pass any headers the proxy needs to authorize the caller (or `[:]` if none).
- `.appAttest(clientID:)` — production without your own backend. Each install authenticates via App Attest and usage is billed to your Anthropic workspace. Add the App Attest capability, register the app in Claude Console under **App integrations**, and pass the `clid_...` client ID. Requires a physical device, so keep `.apiKey` for the Simulator.
</Callout>
...
I created [GiftGenieExample](https://github.com/artemnovichkov/GiftGenieExample) with all the code from this post, so you can run it in Xcode right away. Happy gifting!
