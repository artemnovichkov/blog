...
I've never implemented Picture in Picture mode in my apps, so I decided to recreate eyePhone app from the reel. Initially, I thought it's a simple task. I checked the documentation and open-sourced examples, and thought I vibe-coded it easily. But it turned out to be quite challenging.

I decided to start with a UIKit implementation, because there is no native support of camera capture in SwiftUI.

## Showing camera feed

Before implementing PiP, we need to set up the camera feed. `AVFoundation` framework provides everything we need through `AVCaptureSession`.

Let's start by requesting access to the camera:
...
This checks if we have permission to use the camera and requests it if necessary. The `async` property allows us to use modern Swift concurrency for this permission check.
...
To show the camera feed, we'll use `AVSampleBufferDisplayLayer`, which is optimized for displaying live video. Let's create a view that will use it:
...
We're using `Task` to handle the asynchronous permission check. Once we have permission, we configure the session and start it running on a dedicated queue.

If we run the app now, we should see the camera feed.

## Implementing Picture in Picture

Now that our camera feed is working, let's implement the PiP feature. First, we need to configure our app to support PiP mode.
...
By default, the PiP controller shows playback controls: fast forward, rewind, and play/pause. For a camera feed, these don't make sense, so we'll hide them. According to documentation, you should set `requiresLinearPlayback` to `true`:
...
Now we need to implement the `AVPictureInPictureSampleBufferPlaybackDelegate` protocol to handle the playback:
...
Now, let's run the app and swipe up to start PiP mode.

## Conclusion

Implementing PiP mode in iOS is a bit tricky, there are a lot of nuances to consider. I hope this post will save you some time if you need to implement it in your app.
