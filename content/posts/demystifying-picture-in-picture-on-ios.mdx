...
I decided to start with a UIKit implementation, because there is no native support for camera capture in SwiftUI.
...
To display the camera feed, we'll use `AVSampleBufferDisplayLayer`, which is optimized for live video. Let's create a view that uses it:
