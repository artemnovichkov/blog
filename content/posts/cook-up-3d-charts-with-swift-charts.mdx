...
- [Github contribution graph with Swift Charts](https://artemnovichkov.com/blog/github-contribution-graph-swift-charts)
- [Drawing maps with Swift Charts](https://artemnovichkov.com/blog/drawing-maps-with-swift-charts)
...
`Chart3D` supports 3 types of marks:

- PointMark;
- RuleMark;
- RectangleMark.

It also supports `SurfacePlot` for rendering mathematical surfaces defined by a function of two variables.

We'll use `PointMark` that will represent a single point in 3D space. Since iOS 26.0, we can use a new initializer with three values:
...
The interesting thing here is that we can't set string literals directly, the labels won't be displayed. It looks like a bug for me, but I found this workaround in Code section of related WWDC session.
