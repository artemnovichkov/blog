---
title: Using TextEditor in SwiftUI
description: Text editing
cover: /images/using-text-editor-in-swiftui/cover.png
date: '2024-11-09'
author:
  - artem-novichkov
---

One of the common views in iOS apps is a editable text view. It's a versatile element that can be used for various purposes: leave a comments on social media, create a note, or even write a code. SwiftUI has a basic view for long-form text editing - `TextEditor`. To be honest, it's not as powerful as `UITextView` but it's enough for most cases. 

<Tweet id="1275806023236882432" />

In this article, we'll explore the basic features of `TextEditor` and how to extend it with additional functionality. It will be a first article in a small series about text editing in SwiftUI. Next, we'll cover more topics like text classification and creating your own machine learning model for text analysis. Text is not just symbols, right?
...
Let's start with initializations. `TextEditor` requires a binding to a string:
...
```
Since iOS 18.0 you can handle selection in the text. It may be useful if you offer text suggestions. For instance, you can suggest to change text with LLM like GPT-4o: adjust the length, change reading level, etc.
...
To change the background color, you can hide the scroll content background and set the color:
...
With `focused` modifier we can manage the focus state. For example, if you may show the keyboard on appearance or hide it on saving action.
...
That's what stops me from using `TextEditor` sometimes - it doesn't have a built-in way to disable editing. The first thing that comes to mind is to show `Text` instead of `TextEditor` in read-only mode. It may be useful but users can select only the full text, not a part of it. An other way to disable editing is to use `.constant(text)` binding. The idea looks promising, but the keyboard and the cursor are appearing on tap action, that's not what we want. 
...
Apple has introduced a new feature called Writing Tools. It's a set of tools that help users to write smarter:
...
There is no perfect solution, and you choose the best one based on your requirements. Writing a code I try to found flexibility without complexity. It may be tricky sometimes, but it's worth it.

## What's next?

In this article, we've covered the basic features of `TextEditor`. But it just a beginning. What's really interesting is a meaning of the text. What users want to say? What's the sentiment? How to classify the text? We'll cover these topics in the next articles. Stay tuned!