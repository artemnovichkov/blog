...
`NLEmbedding` is language-specific, so we must set the language. The set of supported languages is limited, for instance, it doesn't support Russian or Kazakh languages. If you try to create an embedding for an unsupported language, you'll get a runtime error. We'll handle it in the next section, so let's continue with English. We can check the vectors of our notes:

```swift
print(sentenceEmbedding.vector(for: "Day was good"))
// prints [-0.15248329937458038, 0.4077046811580658...0.32297080755233765, 0.2538180649280548]
print(sentenceEmbedding.vector(for: "Day is not so good"))
