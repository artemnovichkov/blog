...
In 2022 Apple introduced a brand new SwiftUI framework — [Swift Charts](https://developer.apple.com/documentation/charts). You can use it to visualize data with rich customization and minimal code. Swift Charts is available in iOS 16+ and support watchOS and macOS as well.
In this article I'll try to recreate Github Contribution Graph. Let's start!
...
<Image
  ...
  src={`/images/github-contribution-graph-swift-charts/contribution-graph.png`}
  width={1466}
  height={288}
/>

Basically, it's a [heatmap](https://en.wikipedia.org/wiki/Heat_map) with the following parameters:
...
I was not so active on Github last months 😅, so let's just mock contribution data for last 60 days with random contributions:
...
To visualize this data, we'll import Charts framework and prepare a content view:
...
`RectangleMark` has a few intializers, this one allows us to set x and y range. It's convenient for heatmap chart layout. Every parameter is a `PlottableValue`, a labeled data struct. For x values we use the date value and combine it by week of year. For y values we convert the date to a weekday. I prefer to start my week from Monday, so I'll adjust it:
...
<Image
  alt={`Base chart`}
  src={`/images/github-contribution-graph-swift-charts/base-chart.png`}
  width={400}
  height={867}
/>

Great result for a few lines of code! Chart customization is based on modifiers, so let's add some to represent the count data:
...
`clipShape` modifier allows to customize the appearance of the chart content, and `foregroundStyle` modifier represents the count data:

<Image
  alt={`Chart with styling`}
  src={`/images/github-contribution-graph-swift-charts/color-chart.png`}
  width={400}
  height={867}
/>

Much better! The chart is already useful and represents the data. It even shows a legend based on count range. To make the marks square we can use `chartPlotStyle` modifier. It allows to change frame or aspect ratio of a plot area:
...
<Image
  alt={`Chart with square layout`}
  src={`/images/github-contribution-graph-swift-charts/square-chart.png`}
  width={400}
  height={361}
/>
...
<Image
  alt={`Chart with colors`}
  src={`/images/github-contribution-graph-swift-charts/square-color-chart.png`}
  width={400}
  height={361}
/>
...
Finally, let's add a color-coded legend to indicate the range of contribution values, from "Less" to "More":
...
It overrides the default legend with a custom one. You can also change the legend position like `.chartLegend(position: .bottomTrailing)`, but with this code the legend is placed at the top left corner of the plot and ovelap it. Looks like a bug for me, so let's leave a default position and see the final result:

<Image
  alt={`Final chart`}
  src={`/images/github-contribution-graph-swift-charts/cover.png`}
  width={1200}
  height={740}
/>
